...
export class BlueprintService {
  ...
  private normalizeMicrodramaEpisodeCount(count?: number): 30 | 60 | 100 {
    return count === 60 || count === 100 ? count : 30;
  }

  private getMicrodramaMacroPlans(episodeCount: 30 | 60 | 100): Array<{
    index: number;
    start: number;
    end: number;
  }> {
...
卡点定位：说明这是对应集数内的核心卡点与商业卖点
目的：……
技法卡：……
一级结构：……
承载主线：明确属于哪条事业线/爱情线，起什么作用
情绪骨架：压抑期……；爆发期……
钩子设计：至少写清本段的集首钩子、集尾钩子，以及本中故事最后一集黑场悬念
商业要素：身份要素 / 关系要素 / 欲望要素
节点：……
详细剧情：必须写清对应集数内每一集的关键冲突、爆点、反转和集尾钩子，以及本中故事最后一集切黑时抛出的更大危机
...
11. 每个中故事包含开端、发展、高潮、转折、结局，并标明所属类别与在事业线/爱情线中的作用。
12. 避免简单套路，中故事需有复杂冲突、多层矛盾与主题探讨。
13. 每个中故事结尾必须追加「阶段状态小结」，写清：主角当前状态、主角和主要人物的关系、主角目前受到的压力、主角下一阶段目标方向；并必须额外留下可推进下一中故事的扣子。
14. 整体按约 40 个中故事支撑 200 万字长篇小说；本次只生成第 ${batchIndex} 批 10 个中故事。${isFinalBatch ? '本批必须完成全书终局、主线收束和人物关系落点。' : '本批结尾要留下可继续生成下一批的推进空间，不能写成全书大结局。'}
15. **格式要求：** 每个中故事用明确标题标记，必须从【中故事${this.getChineseNumber(startMacroStoryNumber)}】开始，到【中故事${this.getChineseNumber(endMacroStoryNumber)}】结束，格式如下：
...
5. ${mode === 'microdrama'
          ? '按爆款微短剧中故事设计：必须承接当前中故事已标注的对应集数，内部每集都要有惊艳开场、快节奏推进、打压、高潮、反转、打脸和最后一集黑场钩子；女频内容要强化爱情线桥段、打情骂俏、男女主互动调戏、试探拉扯和情感误会，但不得越过当前关系阶段；结尾必须追加「阶段状态小结」。'
          : '按小说中故事设计：默认能继续拆成10个小故事、20章左右；首个中故事以生死为局开头，后续中故事以重大危局开头，内部要有完整目标、阻碍、升级、高燃点/爽点释放、阶段高潮、结尾扣子和阶段收束。'}
...
13. 结尾必须切在更大的危机、秘密揭露、身份反转、生死倒计时、暧昧误会升级或关系爆雷上，形成下一集黑场钩子。
14. 衔接要求：如果提供了“上一集结尾内容”，本集开头必须从该结尾自然续写，延续同一场景/动作/对话，不要回顾式重述。
