...
请直接输出最终大纲，不要解释过程。`;
  ...
  async generateInspiration(dto: GenerateOutlineDto) {
    ...
    try {
      ...
      const requiresSpecialPower = !isLiteraryWork && dto.requiresSpecialPower !== false;
      const finalFieldName = isLiteraryWork ? '文学核心' : requiresSpecialPower ? '金手指设定' : '';
      ...
      const finalFieldFormat = finalFieldName
        ? `\n${finalFieldName}：\n[详细描述${isLiteraryWork ? '文学气质、叙事特色和主题余韵' : '主角的能力'}]\n`
        : '';
      // 使用详细的Prompt，生成5个架构
...
5. 新增内容必须与故事大纲和既有设定一致，并能直接服务后续人物设定与情节生成。`;
    ...
    const prompt = isMicrodramaWorldSetting
...
${characterNameRestrictions}`;
    ...
    const prompt = `基于以下故事大纲和世界观基础设定，生成一套可直接支撑长篇创作的人物群像。
...
请直接输出本批次的 10 个中故事细纲，不要先列出中故事名称列表。每个中故事的「详细剧情」必须详细具体，可作为 15 章内容的框架基础；前置信息只作短标注，事业线/爱情线信息要服务剧情，不要喧宾夺主。`;
    ...
    try {
	  ...
	  private async regenerateSelectedDetailedOutlineSegments(dto: GenerateDetailedOutlineDto, mode: 'novel' | 'microdrama' | 'literature') {
	    const existingDetailedOutline = dto.existingDetailedOutline?.trim() || '';
	    ...
	    const prompt = `请只针对用户选中的中故事做“局部细化重写”，不要重写未选中的中故事。
...
重写要求：
1. 只输出用户选中的这些中故事，编号必须保持原编号，例如【中故事三】、【中故事四】；禁止输出未选中的中故事，禁止追加新中故事。
2. 每个被选中的中故事都要比原版更具体、更可执行，详细剧情必须占70%以上，不能压缩成摘要。
3. 必须承接上一个中故事的结尾状态、人物关系、当前压力和目标方向；如果下一个中故事存在，结尾必须自然对齐它的开局前提，不能提前消耗它的核心爆点。
4. 按三密度校准：情绪密度更高，桥段密度更高，要素融合更自然；用具体动作、代价、选择、反转和关系变化推进，不能只写概念。
5. 保留「阶段状态小结」，写清：主角当前状态、主要人物关系、当前压力、目标方向。
6. 配角和反派不能工具人化，重要角色必须有欲望、弱点、利益关系和能推动剧情的行动逻辑。

输出格式：
${targets.map(index => `【中故事${this.getChineseNumber(index + 1)}】标题\n详细剧情：……\n阶段状态小结：……`).join('\n\n')}

请直接输出局部细化后的中故事正文，不要输出说明、差异对比或完整大纲。`;
...
以此类推，严格到【中故事十】结束。请直接输出完整文学作品细纲。`;
	  ...
	  private async regenerateDetailedOutlineWithSuggestion(dto: GenerateDetailedOutlineDto, mode: 'novel' | 'microdrama' | 'literature') {
	    const existingDetailedOutline = dto.existingDetailedOutline?.trim() || '';
    ...
    try {
	  ...
	  async generateMicroStories(dto: GenerateMicroStoriesDto) {
	    console.log(`开始为中故事${dto.storyIndex}生成小故事细纲`);
		    ...
		    const mode: 'novel' | 'microdrama' | 'literature' =
		      dto.mode === 'microdrama' ? 'microdrama' : dto.mode === 'literature' ? 'literature' : 'novel';
    const unitLabel = mode === 'microdrama' ? '集' : mode === 'literature' ? '章' : '章';
    const rangeInfo = dto.chapterRange
      ? `，对应${mode === 'microdrama' ? '微短剧集数范围' : '小说章节范围'}：第${dto.chapterRange}${unitLabel}`
      : '';
    const rangeUnitCount = mode === 'literature'
      ? 4
      : dto.chapterRange
...
2. 每个小故事都必须包含完整但轻量的情节发展：开场危机→一次推进→一个爽点/反转→结尾钩子；不要写成2-3分钟的厚剧情
3. 与中故事的主线情节紧密关联
4. 展现不同的叙事角度和人物成长
5. 包含必要的场景、人物动作、关键对白、冲突和转折，但必须精炼；每集只保留1个核心场景，最多1个短转场
      ...
      (async () => {
        try {
          ...
          for (let storyIndex = 0; storyIndex < loopCount; storyIndex++) {
            ...
            const storyPrompt = this.buildMicrodramaEpisodePrompt(storyContext, storyStartChapter, previousEnding, storyData, dto.actionFirstScript, dto.targetEpisodeWords);
