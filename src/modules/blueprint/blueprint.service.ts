...
三、微短剧节奏铁律（必须强执行）：
1. 首个中故事必须以“生死为局”开头：主角一入场就面对生死存亡、社会性死亡、亲密关系毁灭、身份被夺或不可逆失败的极限局面，且在最危急时立刻激活金手指/核心反转/身份反转。但这个开局不能是匿名危机，必须在危机推进中简短带出主角身份、所在时代/城市/世界空间、当前处境和最关键世界规则，让观众知道“这个人是谁、为什么会被逼到这里”。
...
商业要素：只列关键词
详细剧情：
第1集：80-130字，必须写清开场危机、关键动作、一次冲突推进、一个爽点/反转、集尾钩子。
...
不要输出额外说明。`;
  ...
  private buildMicrodramaEpisodePrompt(
    ...
    const firstEpisodeSetupRequirement = episodeNumber === 1
      ? `\n首集开场特别要求（必须执行）：\n- 第1集仍然要有强事件/强压迫抓人，但不能写成匿名主角突然濒死、追杀或灾难现场。\n- 开场前2-3个镜头内，必须用可拍动作、环境细节或极短对白带出：主角姓名/身份、当前社会位置或家庭/职业处境、所处时代/城市/世界空间、这个世界最关键的一条规则或压力。\n- 背景说明要嵌入危机中，例如工牌、债务单、祭坛规则、校园广播、公司会议、家族宴席、系统提示、群众议论、新闻字幕等；不要停下来旁白讲设定。\n- 危机必须让观众明白“为什么这件事会毁掉这个人”，而不是只写生死刺激。\n`
      : '';
...
7. 开场第一场必须直接进入本集主冲突：可以有冲突、羞辱、生死压力、身份失衡、强压局面、暧昧误会、关系爆雷或危险逼近，但必须同时交代主角处境和本集行动目标，不能只给孤立刺激。
